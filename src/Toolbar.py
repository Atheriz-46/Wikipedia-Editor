...
class Toolbar(tk.Frame):
    ...
    def __init__(self, parent, guimanager):
        ...
        self.btn_home = ttk.Button(
            master=self, text="Home", command=self.manager.switchToHomePage
        )
        ...
        self.btn_render = ttk.Button(
            master=self.frm_edit, text="Save", command=(self.manager).frameEditor.render
        )
        self.btn_render.pack(side=tk.LEFT)
        self.btn_save = ttk.Button(
            master=self.frm_edit, text="Exit", command=self.manager.switchToViewer
        )
        self.btn_save.pack(side=tk.LEFT)

        self.btn_edit = ttk.Button(
            master=self.frm_view, text="Edit", command=self.manager.switchToEditor
        )
        ...
        self.btn_new_home = ttk.Button(
            ...
            text="New Page",
            command=lambda: self.newPage(self.frm_home),
        )

        self.ent_view = tk.Entry(master=self.frm_view)
        self.btn_new_view = ttk.Button(
            master=self.frm_view,
            text="New Page",
            command=lambda: self.newPage(self.frm_view),
        )
